/**
 * FILE: test_picokit_29_lcd_menu_remote_and_security.c
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 Picokit LCD menu remote node:
 ...
 * menu state machine that pairs each item with an authenticated heartbeat.
 *
 * BRIEF:
 * Native unit test runner for picokit-29-lcd-menu-remote.
 *
 * AUTHOR: Kevin Thomas
 * DATE: September 2026
 */
...
static void nec_arm_at(uint8_t command, uint64_t base) {
    ...
    mock_gpio_timeline_begin_at(base, s_ir_off, s_ir_lvl, count,
                                PICOKIT_29_LCD_MENU_REMOTE_IR_PIN);
}
...
static void assert_pin_constants(void) {
    TEST_ASSERT_EQUAL_UINT(25u, PICOKIT_29_LCD_MENU_REMOTE_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(5u, PICOKIT_29_LCD_MENU_REMOTE_IR_PIN);
    TEST_ASSERT_EQUAL_UINT(8u, PICOKIT_29_LCD_MENU_REMOTE_UART_TX);
    TEST_ASSERT_EQUAL_UINT(9u, PICOKIT_29_LCD_MENU_REMOTE_UART_RX);
}
...
static void assert_frame_constants(void) {
    TEST_ASSERT_EQUAL_UINT(115200u, PICOKIT_29_LCD_MENU_REMOTE_UART_BAUD);
    TEST_ASSERT_EQUAL_UINT(48u, PICOKIT_29_LCD_MENU_REMOTE_FRAME_SIZE);
    TEST_ASSERT_EQUAL_UINT(5000u, PICOKIT_29_LCD_MENU_REMOTE_TX_INTERVAL_MS);
...
static void assert_artifact_ids(void) {
    ...
    TEST_ASSERT_EQUAL_UINT(29u, PACKET_NODE_ID);
...
/**
 * @brief Arm and step one NEC menu key at an absolute time.
 *
 * @param key Eight-bit remote command code.
 * @param when Absolute fake-clock time in microseconds.
 * @return void
 */
static void menu_step(uint8_t key, uint64_t when) {
...
void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[PICOKIT_29_LCD_MENU_REMOTE_IR_PIN]);
}
...
void test_ir_poll_stuck_high(void) {
    ir_command_t cmd;
    reset_all();
    gpio_put(PICOKIT_29_LCD_MENU_REMOTE_IR_PIN, true);
    TEST_ASSERT_FALSE(ir_remote_poll(&cmd));
}
...
void test_monitor_build_frame_json(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("{\"n\":29,\"s\":12,\"m\":1}", frame);
}
...
void test_monitor_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_29_LCD_MENU_REMOTE_LED_PIN]);
...
void test_monitor_menu_up(void) {
    init_monitor();
    menu_step(0x45u, 1000000u);
    TEST_ASSERT_EQUAL_UINT8(1u, g_menu);
    TEST_ASSERT_EQUAL_STRING("MENU 2/3", g_line1);
    TEST_ASSERT_EQUAL_STRING("[SET]", g_line2);
}

void test_monitor_menu_down(void) {
    init_monitor();
    menu_step(0x46u, 1000000u);
    TEST_ASSERT_EQUAL_UINT8(2u, g_menu);
    TEST_ASSERT_EQUAL_STRING("MENU 3/3", g_line1);
    TEST_ASSERT_EQUAL_STRING("[INFO]", g_line2);
}

void test_monitor_menu_ok(void) {
    init_monitor();
    menu_step(0x47u, 1000000u);
    TEST_ASSERT_EQUAL_UINT8(0u, g_menu);
    TEST_ASSERT_EQUAL_UINT8(0x47u, g_key_code);
}
...
void test_display_init(void) {
    reset_all();
    TEST_ASSERT_TRUE(display_init(i2c1, PICOKIT_29_LCD_MENU_REMOTE_LCD_ADDR));
    TEST_ASSERT_TRUE(mock_i2c_log_count() > 0u);
}
...
void test_display_init_fails(void) {
    reset_all();
    mock_i2c_set_write_fail(true);
    TEST_ASSERT_FALSE(display_init(i2c1, PICOKIT_29_LCD_MENU_REMOTE_LCD_ADDR));
}
...
void test_display_format(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("N:29 S:0042 OK", s_line2);
}
...
void test_display_format_negative(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("N:29 S:0000 !!", s_line2);
}

void test_display_render(void) {
    reset_all();
    strcpy(s_line1, "MENU 2/3");
    strcpy(s_line2, "[SET]");
    display_render_lines(i2c1, PICOKIT_29_LCD_MENU_REMOTE_LCD_ADDR, s_line1, s_line2);
...
/**
 * @brief Run the infrared menu monitor state-machine tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_tests(void) {
    ...
    RUN_TEST(test_monitor_menu_up);
    RUN_TEST(test_monitor_menu_down);
    RUN_TEST(test_monitor_menu_ok);
